...
const isInstance = (instance) => {
  if (instance) {
    const isValidInstance =
      lodash.isFunction(lodash.get(instance, 'toObject', null)) &&
      !lodash.isNull(lodash.get(instance, '$__', null));
...
const modelNames = (connection) => {
  ...
  const registeredModelNames = common.sortedUniq(
...
const createSubSchema = (definition, optns) => {
  ...
  const schemaDefinition = common.mergeObjects(definition);

  // ensure sub schema options
  const schemaOptions = common.mergeObjects(SUB_SCHEMA_OPTIONS, optns);
...
const createVarySubSchema = (optns, ...paths) => {
  ...
  const fields = lodash.map([...paths], (field) => {
    ...
    if (lodash.isPlainObject(field)) {
      return common.mergeObjects({ required: false }, field);
    }
  ...
  const definition = {};
  lodash.forEach(common.sortedUniq([...fields]), (field) => {
    definition[field.name] = common.mergeObjects(options, lodash.omit(field, 'name'));
  });
...
const createSchema = (definition, optns, ...plugins) => {
  ...
  const schemaOptions = common.mergeObjects(SCHEMA_OPTIONS, optns);
...
const model = (modelName, schema, connection) => {
  ...
  localModelName = lodash.isString(modelName) ? modelName : localModelName;
  ...
  const modelExists = lodash.includes(localModelNames, localModelName);
...
const collectionNameOf = (connection, modelName) => {
  ...
  let collectionName =
    lodash.get(Model, 'collection.name') || lodash.get(Model, 'collection.collectionName');

  // derive collection from model name
  if (lodash.isEmpty(collectionName)) {
...
const deleteModels = (connection, ...models) => {
  // ensure connection
  let localConnection = lodash.find([connection, ...models], (modelName) => {
    return isConnection(modelName);
  });
  localConnection = localConnection || mongoose.connection;

  // ensure valid model names
  let localModelNames = lodash.filter([connection, ...models], (modelName) => {
    return lodash.isString(modelName) || isModel(modelName);
  });
  localModelNames = common.uniq([...localModelNames]);

  // use all models is non provided
  if (lodash.isEmpty(localModelNames)) {
    localModelNames = common.uniq([
      ...localModelNames,
  ...
  lodash.forEach(localModelNames, (modelName) => {
...
const createModel = (schema, options, connection, ...plugins) => {
  ...
  const schemaDefinition = common.mergeObjects(schema);

  // ensure model options with defaults
  const modelOptions = common.mergeObjects(options, SCHEMA_OPTIONS);

  // obtain connection if provided
  const localConnection = lodash.find([connection, ...plugins], (plugin) => {
    return isConnection(plugin);
  });

  // ensure valid plugins
  const allowedPlugins = common.compact(
    lodash.filter([connection, ...plugins], (plugin) => {
      return !isConnection(plugin);
    })
  );
...
const connect = (url, done) => {
  ...
  try {
    DB_NAME = lodash.get(common.pkg(), 'name', NODE_ENV);
    DB_NAME = lodash.toLower(lodash.last(lodash.split(DB_NAME, '/')));
    DB_NAME = DB_NAME === NODE_ENV ? DB_NAME : `${DB_NAME} ${NODE_ENV}`;
    DB_NAME = lodash.kebabCase(DB_NAME);
  } catch (error) {
    /* ignore */
  }
  ...
  const MONGODB_URI = lodash.trim(env.getString('MONGODB_URI', DB_NAME));

  // normalize arguments
  const uri = lodash.trim(lodash.isFunction(url) ? MONGODB_URI : url);
  const cb = lodash.isFunction(url) ? url : done;
  ...
  const reply = (error, conn) => {
    ...
    const waitTime = env.getNumber('MONGODB_CONNECTION_WAIT_TIME');

    // wait before reply
    if (waitTime) {
      setTimeout(() => {
        common.wrapCallback(cb)(error, conn);
      }, waitTime);
    }
...
const disconnect = (connection, done) => {
  ...
  if (localConnection) {
    localConnection.close(common.wrapCallback(cb, replyConnection));
  }
  // disconnect all connection
  else {
    mongoose.disconnect(common.wrapCallback(cb, replyConnection));
  }
};
...
const create = (...instances /* , done */) => {
  // ensure callback
  const cb = lodash.findLast([].concat(...instances), (instance) => {
    return lodash.isFunction(instance) && !isInstance(instance);
  });

  // collect provided instances
  let localInstances = lodash.filter([].concat(...instances), (instance) => {
    return isInstance(instance);
  });

  // compact and ensure unique instances by _id
  localInstances = lodash.uniqBy(common.compact([...localInstances]), '_id');

  // map instances to save
  // TODO: use insertMany for same model instances
  // const connected = isConnected();
  let saves = lodash.map([...localInstances], (instance) => {
    const save = (next) => {
  ...
  // compact saves
  saves = common.compact([...saves]);

  // save & return
  return async.parallel(saves, cb);
};
...
const syncIndexes = (connection, done) => {
  ...
  const localModelNames = modelNames(localConnection);
  const Models = lodash.map(localModelNames, (modelName) => {
    return model(modelName, localConnection);
  });

  // build indexes sync tasks
  let syncs = lodash.map(Models, (Model) => {
    return syncIndexesOf(Model);
  });

  // do syncing
  syncs = common.compact([...syncs]);
  return async.parallel(syncs, (error) => cb(error));
};
...
const clear = (connection, ...models /* , done */) => {
  ...
  // ensure callback
  const cb = lodash.findLast([connection, ...models], (modelName) => {
    return lodash.isFunction(modelName);
  });
  ...
  const clearModel = (modelName) => {
    ...
    // prepare cleaner
    if (connected && Model && lodash.isFunction(Model.deleteMany)) {
  ...
  let deletes = lodash.map([...localModelNames], clearModel);
  deletes = common.compact([...deletes]);

  // run deletes
  return async.waterfall(deletes, cb);
};
...
exports.isConnectedOrConnecting = isConnectedOrConnecting;
