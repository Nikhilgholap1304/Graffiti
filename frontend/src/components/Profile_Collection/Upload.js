...
const Upload = () => {
  ...
  const handleOpenFullyDesign = async (designId) => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        setTimeout(() => {
          setHasDesignerBadge(res.data.ShortUserDetails[2]);
        }, 200);
        setTimeout(() => {
  ...
  useEffect(() => {
    const fetchUploadedDesignData = async () => {
      try {
        const config = {
          headers: {
            Authorization: sessionToken,
        ...
        const res = await axios.get('/fetch_design_data', config);
        if (res.status === 401) {
          console.log("Unauthorized access without session");
          ...
          setHasDesignerBadge(res.data.hasDesignerBadge);
  ...
  return (
    <>
                  ...
                  <Box sx={{ width: '100%' }}>
                    <Collapse in={designerBadgeInfoOpen}>
                      <Alert
                        ...
                        sx={{ mb: 2 }}
                      >
                        Hello {name}, If you wish to get the designer badge, you need to upload 3 or more designs.
                      </Alert>
                    </Collapse>
                    {/* <Button
                  disabled={open}
                      ...
                      <>
                        ...
                        <div className="CardLowerPart">
                          <div className='CardUploadersName&PicCont'>
                            ...
                            >
                              <Avatar alt="Remy Sharp" src={profilePic || Default_panda} />
                            </StyledBadge>
                  ...
                  <div className="sticky_header_user_cont">
                    {isFODLoading ? (
                      ...
                      <StyledBadge
                        ...
                        variant={hasDesignerBadge === true ? "dot" : "standard"}
                      >
                        <Avatar alt="Remy Sharp" src={singleDesignUD[1] || Default_panda} />
                      </StyledBadge>
                    )}
      ...
      <section className={`signout_all_popup confirmdelete ${confirmDelActive ? 'active' : ''}`}>
