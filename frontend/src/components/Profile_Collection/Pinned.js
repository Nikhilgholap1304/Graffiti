...
const Pinned = () => {
  ...
  useEffect(() => {
    const fetchPinnedDesignData = async () => {
      try {
        const config = {
          headers: {
            Authorization: sessionToken,
        ...
        const res = await axios.get('/fetch_pinned_design_data', config);
        if (res.status === 401) {
          console.log("Unauthorized access without session");
            ...
            <>
              <section className="blank_msg">
                ...
                <h1 className="EmptyMsg">
                  It looks like you haven't pinned any of the designs,<br></br> To save the pinned designs,search the designs and press the pin button.
                </h1>
              </section>
            </>
          )
                ...
                <div className="sticky_header_like_message_cont">
                  {isFODLoading ? (
                    ...
                    <>
                      ...
                      <IconButton className='Sticky_Ico' title='put a comment' 
                      onClick={() => handleMessageClicked(singleDesign._id)
                      }>
                        <CommentIcon />
                      </IconButton>
                    </>
