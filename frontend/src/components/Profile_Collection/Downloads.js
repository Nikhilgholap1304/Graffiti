...
const Downloads = () => {
  ...
  useEffect(() => {
    const fetchDownloadedDesignData = async () => {
      try {
        const config = {
          headers: {
            Authorization: sessionToken,
        ...
        const res = await axios.get('/fetch_downloaded_design_data', config);
        if (res.status === 401) {
          console.log("Unauthorized access without session");
            ...
            <>
              <section className="blank_msg">
                ...
                <h1 className="EmptyMsg">
                  It looks like you haven't downloaded any of the designs,<br></br> To save the downloaded designs, press the download button within it.
                </h1>
              </section>
            </>
          )
