...
const ReportedData = () => {
  ...
  const [singleDesignReports, setSingleDesignReports] = useState([]);
  ...
  const handleOpenFullyDesign = async (designId) => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        setSingleDesignReports(res.data.reasonCounts);
        setTimeout(() => {
  ...
  useEffect(() => {
    const fetchDesignData = async () => {
      try {
        const res = await axios.get(`/fetch_admin_reported_design_data`)
  ...
  const isDesigner = (designUserId) => {
    ...
    console.log(designerUsers.some(user => user.userId === designUserId))
  };
                        ...
                        {isDesignLoading ? (
                          ...
                          <>
                            ...
                            <div className={DesignStyle.CardLowerPart}>
                              <div className={DesignStyle['CardUploadersName&PicCont']}>
                                <StyledBadge
                                  overlap="circular"
        ...
        <div className={DesignStyle.FullyOpenDesignSection} ref={StickyHeaderFODRef}>
          <div className={DesignStyle.InnerFullyOpenDesign} onClick={(e) => e.stopPropagation()}>
            <div className={DesignStyle.InnerFullyOpenDesignCont}>
              ...
              {isFODLoading ? (
                <Skeleton variant="rectangular" sx={{
                  borderRadius: '5px', width: "100%", height: "50px", borderTopLeftRadius: '0rem',
                ...
                <Accordion sx={{
                  ...
                  <AccordionDetails sx={{ display: 'flex', flexWrap: "wrap", gap: 3 }}>
                    {Object.entries(singleDesignReports).map(([reason, count]) => (
                      <Badge badgeContent={count} color="primary" key={reason}>
                        <Chip variant="outlined" color="warning" label={reason} className={DesignStyle.Tag_Chips} />
                      </Badge>
                    ))}
                  </AccordionDetails>
                </Accordion>
