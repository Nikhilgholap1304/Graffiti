...
const DesginersData = () => {
  ...
  useEffect(() => {
    const fetchDesigners = async () => {
      try {
        const res = await axios.get('/admin/fetch_designers_data'); // Replace '/api/users' with your actual API endpoint
        setUsers(res.data);
...
const exportExcel = () => {
  import('xlsx').then((xlsx) => {
    ...
    saveAsExcelFile(excelBuffer, `designers`);
  });
};
        ...
        >
          ...
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Link
              underline="hover"
              color="text.primary"
              onClick={() => Navigate("/Admin/DesignersData")}
              aria-current="page"
              sx={{ cursor: "pointer" }}
            >
              Designers
          ...
          <div className="card p-fluid" style={{ fontSize: "initial" }}>
            ...
            <DataTable ref={dt} value={users} editMode="row" frozenValue={lockedUsers} paginator rows={10} removableSort dataKey="id" filters={filters} filterDisplay="row" loading={loading} header={header} emptyMessage="No designers found." onRowEditComplete={onRowEditComplete} tableStyle={{ minWidth: '50rem' }} selectionMode="single" selection={selectedUser} onSelectionChange={(e) => setSelectedUser(e.value)} >
