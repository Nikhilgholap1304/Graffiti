...
import "./Styles_Category/DefaultCategory.css";
...
const DefaultCategory = ({ TagName }) => {
  ...
  useEffect(() => {
    const fetchDesignData = async () => {
      try {
        const res = await axios.get(`/fetch_tag_design_data/${TagName}`)
    ...
    setUploadedDesign, openFullyDesign, setSingleDesign, handleToggleLike, TagName
  ])
  ...
  return (
    <>
      <Header />
      <section className={`search_header ${isSearchHeaderVisible ? 'active' : ''}`}>
      ...
      {isEmptyMsgCirLoader ? (
        ...
        <>
          ...
          {!emptyDesign || !uploadedDesign===0 ? (
            <section className="CardSection Home DefaultCategory">
            ...
            <>
              <section className="blank_msg">
                ...
                <h1 className="EmptyMsg">
                  It looks like {TagName} category can't be found
                </h1>
              </section>
            </>
          )
